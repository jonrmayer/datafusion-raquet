...
impl ScalarUDFImpl for TestFromTile {
...
fn build_cell_array(arrays: Vec<ArrayRef>) -> RaquetDataFusionResult<BinaryArray> {
...
//         ctx.register_udf(TestFromTile::default().into());

//         let sql = r#"SELECT test_from_tile(band_1) from solar;"#;
//         // println!("{:?}", sql);
