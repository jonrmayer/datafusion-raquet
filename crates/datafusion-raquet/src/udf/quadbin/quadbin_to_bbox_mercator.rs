...
use quadbin_rs::{Tile, cell_to_tile, tile_to_bbox_mercator};
...
impl Default for QuadBinToBBOXMercator {
    fn default() -> Self {
        Self::new()
    }
}
...
fn build_cell_array(arrays: Vec<ArrayRef>) -> RaquetDataFusionResult<StructArray> {
    ...
    for cell in cells.iter() {
        ...
        let bbox_mercator = tile_to_bbox_mercator(tile);
        let abox: Abbox = Abbox::new(bbox_mercator);
        vcells.push(abox);
    }
