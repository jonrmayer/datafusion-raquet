//! Defines [`RasterArrowError`], representing all errors returned by this crate.
...
pub type QuadbinArrowResult<T> = std::result::Result<T, QuadbinArrowError>;

impl From<QuadbinArrowError> for ArrowError {
    ...
    fn from(err: QuadbinArrowError) -> Self {
        match err {
            QuadbinArrowError::Arrow(err) => err,
