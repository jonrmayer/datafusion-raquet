...
pub type RasterArrowResult<T> = std::result::Result<T, RasterArrowError>;

impl From<RasterArrowError> for ArrowError {
    ...
    fn from(err: RasterArrowError) -> Self {
        match err {
            RasterArrowError::Arrow(err) => err,
