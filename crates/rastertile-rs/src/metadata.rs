...
impl FromStr for CompressionFormat {
    type Err = RasterTileError;
    fn from_str(s: &str) -> RasterTileResult<Self> {
...
impl Metadata {
    ...
    // pub(crate) fn deserialize<S: AsRef<str>>(metadata: Option<S>) -> Result<Self, RasterTileError> {
    //     if let Some(ext_meta) = metadata {
    //         Ok(serde_json::from_str(ext_meta.as_ref())
    //             .map_err(|err| RasterTileError::ExternalError(Box::new(err)))?)
    //     } else {
    //         Ok(Default::default())
    //     }
    // }
