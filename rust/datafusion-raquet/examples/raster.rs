...
pub async fn test_native_tile_1000(ctx: SessionContext) -> Vec<arrow_array::RecordBatch> {
  ...
  with data as (
    SELECT binary_to_raquet(band_1,'256', 'Separated', 'float32','NaN','gzip') band from solar where block<>0 
    limit 1000
    )

    select raquet_band_native(band) native from data"###;
